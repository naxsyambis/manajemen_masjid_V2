...
import { Plus, Edit, Trash2, Eye, AlertTriangle, X, Calendar, User, Search } from 'lucide-react';
...
const DataKepengurusan = ({ user, onLogout }) => {
  ...
  const fetchPengurus = async () => {
    try {
      ...
      alert('Gagal memuat data kepengurusan');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="main-content p-6 min-h-screen overflow-y-auto"> {/* Content area */}
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
              <div>
                <h1 className="text-4xl font-black text-gray-800 mb-2">Data Kepengurusan</h1>
                <p className="text-gray-600">Kelola informasi kepengurusan masjid dengan mudah dan efisien</p>
              </div>
              <button
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
              <div className="bg-mu-green p-6 text-white">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  <div>
                    <h2 className="text-2xl font-bold">Daftar Pengurus</h2>
                    <p className="text-green-100 mt-1">Total: {filteredPengurus.length} pengurus</p>
                  </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Foto</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredPengurus.map((p, index) => (
                      <tr key={p.pengurus_id} className={`hover:bg-gray-50 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
