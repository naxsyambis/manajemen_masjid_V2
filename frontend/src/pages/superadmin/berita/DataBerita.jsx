          ...
          <div className="bg-white p-6 md:p-10 rounded-[3rem] border border-gray-100 shadow-xl shadow-gray-200/50">
              ...
              <table className="min-w-full border-separate border-spacing-0">
                <thead>
                  <tr className="bg-gray-50/50">
                    ...
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Tanggal</th>
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Ringkasan</th>
                      ...
                      <tr key={berita.berita_id} className="hover:bg-gray-50/80 transition-colors group">
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="w-14 h-14 bg-gray-100 rounded-2xl overflow-hidden shadow-inner flex items-center justify-center border-2 border-transparent group-hover:border-mu-green transition-all">
                            {berita.gambar ? (
                              <img src={`http://localhost:3000/uploads/berita/${berita.gambar}`} alt="Gambar" className="w-full h-full object-cover" />
                            ) : (
                        ...
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="text-lg font-bold text-gray-800 group-hover:text-mu-green transition-colors max-w-[200px] truncate" title={berita.judul}>{berita.judul}</div>
                          <div className="text-xs text-gray-400 font-black uppercase tracking-tighter mt-1">ID: {berita.berita_id}</div>
                        </td>
                        <td className="px-6 py-6 border-b border-gray-50">
                        ...
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="text-sm text-gray-600 max-w-[250px] line-clamp-2 leading-relaxed">
                            {berita.isi || "Tidak ada ringkasan"}
                          </div>
                        </td>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-12 gap-6">
              <p className="text-xs font-black text-gray-400 uppercase tracking-widest">
                Menampilkan <span className="text-mu-green">{filteredBeritas.length > 0 ? indexOfFirstItem + 1 : 0}</span> - <span className="text-mu-green">{Math.min(indexOfLastItem, filteredBeritas.length)}</span> dari {filteredBeritas.length} data
              </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] px-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-white/20">
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-base text-gray-500 mb-8 leading-relaxed">Apakah Anda yakin ingin menghapus berita <strong>{selectedBerita.judul}</strong>? Data yang dihapus tidak bisa dikembalikan.</p>
