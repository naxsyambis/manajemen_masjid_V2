...
import { AlertTriangle, Trash2, X, MapPin, Phone, FileText, Image as ImageIcon } from 'lucide-react';

const HapusMasjid = ({ user, onLogout }) => {
              ...
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between">
                <div className="flex items-center space-x-6 mb-6 lg:mb-0">
                  <div className="w-20 h-20 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                    {masjid.logo_foto ? (
                      <img src={`http://localhost:3000${masjid.logo_foto}`} alt="Logo" className="w-16 h-16 object-cover rounded-xl" />
                    ) : (
                      <ImageIcon size={40} className="text-white/80" />
                    )}
                  </div>
                  <div>
                    <h1 className="text-4xl font-bold mb-2">{masjid.nama_masjid}</h1>
                    <p className="text-red-100 text-lg">ID Masjid: {masjid.masjid_id}</p>
                  </div>
                </div>
          ...
          <div className="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-2xl p-8 mb-8 shadow-lg">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-red-600">Menghapus masjid akan menghilangkan semua data terkait secara permanen.</p>
              </div>
            </div>
            <p className="text-red-700 leading-relaxed">
              Pastikan Anda benar-benar ingin menghapus masjid ini. Semua informasi, logo, dan data lainnya akan hilang selamanya dan tidak dapat dikembalikan.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <h3 className="text-xl font-semibold text-gray-800">Alamat Masjid</h3>
              </div>
