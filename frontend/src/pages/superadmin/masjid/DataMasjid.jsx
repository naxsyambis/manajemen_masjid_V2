          ...
          <div className="bg-white p-6 md:p-10 rounded-[3rem] border border-gray-100 shadow-xl shadow-gray-200/50">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
              <div className="flex items-center gap-6 w-full md:w-auto">
                <div className="hidden sm:block">
                  <h3 className="text-lg font-black text-gray-800 uppercase tracking-tighter">Daftar Masjid</h3>
                  <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Total: {filteredMasjids.length}</p>
                </div>
              ...
              <table className="min-w-full border-separate border-spacing-0">
                <thead>
                  <tr className="bg-gray-50/50">
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Logo</th>
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Nama Masjid</th>
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Alamat Lengkap</th>
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Kontak HP</th>
                    <th className="px-6 py-6 text-left text-xs font-black text-gray-400 uppercase tracking-widest border-b-2 border-gray-50">Koordinat</th>
                ...
                <tbody>
                  {currentItems.length > 0 ? (
                    currentItems.map((masjid) => (
                      <tr key={masjid.masjid_id} className="hover:bg-gray-50/80 transition-colors group">
                        ...
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="text-lg font-bold text-gray-800 group-hover:text-mu-green transition-colors">{masjid.nama_masjid}</div>
                          <div className="text-xs text-gray-400 font-black uppercase tracking-tighter mt-1">ID: {masjid.masjid_id}</div>
                        </td>
                        ...
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="flex items-start gap-2 text-sm text-gray-600 max-w-[200px]">
                            ...
                            <span className="leading-relaxed">{masjid.alamat}</span>
                          </div>
                        </td>
                        {/* Kolom Kontak Terpisah */}
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="flex items-center gap-2 text-sm text-gray-800 font-black bg-gray-100/50 w-fit px-3 py-2 rounded-xl">
                            <Phone size={14} className="text-mu-green flex-shrink-0" />
                            <span>{masjid.no_hp}</span>
                          </div>
                        </td>
                        <td className="px-6 py-6 border-b border-gray-50">
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 text-[11px] text-gray-500 font-bold bg-gray-100 px-2 py-1 rounded-md w-fit">
                              <Navigation size={12} className="text-mu-green" />
                    ...
                    <tr>
                      <td colSpan="6" className="text-center py-24 text-gray-400 text-sm font-black uppercase tracking-[0.2em]">Data tidak ditemukan</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-12 gap-6">
              <p className="text-xs font-black text-gray-400 uppercase tracking-widest">
                Menampilkan <span className="text-mu-green">{filteredMasjids.length > 0 ? indexOfFirstItem + 1 : 0}</span> - <span className="text-mu-green">{Math.min(indexOfLastItem, filteredMasjids.length)}</span> dari {filteredMasjids.length} data
              </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[100] px-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden border border-white/20">
            ...
            <div className="p-10 text-center">
              ...
              <p className="text-base text-gray-500 mb-8 leading-relaxed">Apakah Anda yakin ingin menghapus <strong>{selectedMasjid.nama_masjid}</strong>? Data yang dihapus tidak bisa dikembalikan.</p>
