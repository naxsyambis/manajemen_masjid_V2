...
const BeritaSection = () => {
  ...
  useEffect(() => {
    const fetchBerita = async () => {
      try {
        const response = await fetch('http://localhost:3000/public/berita'); // Ganti dengan URL backend Anda
        if (!response.ok) {
  ...
  if (loading) {
    return (
      <section id="berita" className="berita-section fade-in relative">
    ...
    <section id="berita" className="berita-section fade-in relative">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 relative z-10">
          <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#0b2b26] mb-4">Berita Terbaru</h2>
            ...
            beritaTerbaru.map((item) => (
              ...
              >
                <div className="relative h-60 overflow-hidden rounded-t-2xl">
                  <img
                    src={item.gambar || 'https://via.placeholder.com/800x400?text=No+Image'} // Fallback jika gambar kosong
                    alt={item.judul}
