...
const EditBerita = () => {
  ...
  const fetchDetail = async () => {
    try {
      ...
      const gambarList = Array.isArray(data.gambar_list)
        ? data.gambar_list
        : [];

      setExistingImages(gambarList.slice(0, 5));
    } catch (err) {
  ...
  const handleRemoveExisting = (gambarId) => {
    setExistingImages((prev) =>
      prev.filter((img) => img.gambar_id !== gambarId)
    );

    setDeletedIds((prev) =>
      prev.includes(gambarId) ? prev : [...prev, gambarId]
    );
  };
            ...
            <div className="p-10 lg:p-16">
              <div className="mb-12 text-center">
                ...
                <p className="text-gray-600 text-lg">
                  Perbarui informasi berita dengan lengkap dan akurat
                </p>
              </div>
