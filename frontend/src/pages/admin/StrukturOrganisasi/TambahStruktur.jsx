...
const AlertPopup = ({ alertData, onClose }) => {
  // ...(Kode AlertPopup tetap sama seperti sebelumnya)...
...
const TambahStruktur = () => {
  ...
  const [fileTtd, setFileTtd] = useState(null);       // STATE BARU TTD
  const [previewTtdUrl, setPreviewTtdUrl] = useState(null); // STATE BARU PREVIEW TTD
  ...
  const handleAuthError = (err) => {
    if (err.response && err.response.status === 401) { showPopup({ type: 'error', title: 'Sesi Berakhir', message: err.response.data.message || 'Sesi Anda telah berakhir.', confirmText: 'Login Ulang', onConfirm: () => { localStorage.removeItem('token'); window.location.href = '/login'; } }); return true; }
    return false;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/takmir/struktur-organisasi', data, { headers: { Authorization: `Bearer ${token}` } });
      showPopup({ type: 'success', title: 'Data Tersimpan', message: 'Data struktur organisasi berhasil ditambahkan.', onConfirm: () => navigate('/admin/struktur-organisasi') });
      setFormData({ nama: '', jabatan: '', periode_mulai: '', periode_selesai: '' });
      setFile(null); setPreviewUrl(null);
      setFileTtd(null); setPreviewTtdUrl(null);
    } catch (err) {
      if (handleAuthError(err)) return;
      const msg = err.response?.data?.message || 'Gagal menambahkan data struktur organisasi.';
      setError(msg); showPopup({ type: 'error', title: 'Gagal Menyimpan', message: msg });
    ...
    <div className="tambah-struktur min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 animate-fadeIn">
      ...
      <div className="main-content p-8 h-full overflow-y-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-black text-gray-800 uppercase tracking-tighter leading-none">Tambah <span className="text-mu-green">Struktur</span></h1>
          ...
          <div className="flex gap-4"><button onClick={() => window.location.reload()} className="flex items-center gap-2 bg-white border border-gray-100 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-mu-green transition-all shadow-sm active:scale-95 hover:shadow-lg"><RefreshCcw size={14} />Refresh Halaman</button></div>
        </div>
        ...
        <div className="bg-white rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
          <div className="p-10 lg:p-16">
            <div className="mb-12 text-center"><h2 className="text-3xl font-black text-gray-800 uppercase tracking-tighter mb-4">Tambah Struktur Organisasi</h2><p className="text-gray-600 text-lg">Isi informasi pengurus struktur organisasi masjid dengan lengkap.</p></div>
                ...
                <div className="space-y-10">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-gray-500">Upload foto anggota struktur organisasi, opsional.</p>
                    </div>
                  </div>
                      ...
                      <label htmlFor="ttd-upload" className="block">
                        <div className="border-2 border-dashed border-gray-300 rounded-2xl p-10 text-center cursor-pointer hover:border-mu-green hover:bg-mu-green/5 transition-all duration-300 shadow-lg hover:shadow-xl">
                          {previewTtdUrl ? (
                            ...
                            <div className="space-y-6"><PenTool size={64} className="text-gray-400 mx-auto" /><div><p className="text-lg font-semibold text-gray-700">Klik untuk upload TTD</p><p className="text-sm text-gray-500">Background transparan (PNG) lebih disarankan, opsional.</p></div></div>
                          )}
                        </div>
                      </label>
                    </div>
