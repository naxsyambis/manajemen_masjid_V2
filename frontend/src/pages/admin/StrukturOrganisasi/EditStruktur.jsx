...
const EditStruktur = () => {
  ...
  const fetchStruktur = async () => {
    try {
      ...
      if (!masjidId) { showPopup({ type: 'warning', title: 'Masjid ID Tidak Ada', message: 'Silakan logout lalu login ulang.', onConfirm: () => navigate('/admin/struktur-organisasi') }); return; }
      ...
      if (!selected) { showPopup({ type: 'warning', title: 'Data Tidak Ditemukan', message: 'Data struktur organisasi tidak ditemukan.', onConfirm: () => navigate('/admin/struktur-organisasi') }); return; }
  ...
  const handleFileChange = (e, type) => {
    ...
    if (!selectedFile.type.startsWith('image/')) { showPopup({ type: 'warning', title: 'File Tidak Valid', message: 'File harus berupa gambar.' }); e.target.value = ''; return; }
    if (selectedFile.size > 3 * 1024 * 1024) { showPopup({ type: 'warning', title: 'Ukuran Terlalu Besar', message: 'Ukuran file maksimal 3MB.' }); e.target.value = ''; return; }
  ...
  const validateForm = () => {
    ...
    if (formData.nama.trim().length < 3) { showPopup({ type: 'warning', title: 'Nama Terlalu Pendek', message: 'Nama minimal 3 karakter.' }); return false; }
    if (!formData.jabatan.trim()) { showPopup({ type: 'warning', title: 'Jabatan Kosong', message: 'Jabatan wajib diisi.' }); return false; }
    if (formData.periode_mulai && formData.periode_selesai && new Date(formData.periode_mulai) > new Date(formData.periode_selesai)) { showPopup({ type: 'warning', title: 'Periode Tidak Valid', message: 'Periode mulai tidak boleh lebih besar dari periode selesai.' }); return false; }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showPopup({ type: 'success', title: 'Data Diupdate', message: 'Data struktur organisasi berhasil diupdate.', onConfirm: () => navigate('/admin/struktur-organisasi') });
    } catch (err) {
      if (handleAuthError(err)) return;
      const msg = err.response?.data?.message || 'Gagal mengupdate data struktur organisasi.';
      setError(msg); showPopup({ type: 'error', title: 'Gagal Update', message: msg });
    } finally { setLoading(false); }
  };

  if (pageLoading) return <div className="h-screen w-full flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100"><div className="flex flex-col items-center gap-4"><div className="w-16 h-16 border-4 border-mu-green border-t-transparent rounded-full animate-spin shadow-lg"></div><p className="text-sm font-bold text-mu-green uppercase tracking-wider">Memuat Data Struktur Organisasi...</p></div></div>;
    ...
    <div className="edit-struktur min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 animate-fadeIn">
      ...
      <div className="main-content p-8 h-full overflow-y-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-4"><button onClick={fetchStruktur} disabled={refreshing} className="flex items-center gap-2 bg-white border border-gray-100 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-mu-green transition-all shadow-sm active:scale-95 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"><RefreshCcw size={14} className={refreshing ? 'animate-spin' : ''} />{refreshing ? 'Memuat...' : 'Refresh Data'}</button></div>
        </div>

        {error && (<div className="bg-red-50 border border-red-200 rounded-2xl p-6 flex items-center gap-4 shadow-lg"><AlertCircle size={24} className="text-red-500 flex-shrink-0" /><div><p className="text-red-700 font-medium">Error</p><p className="text-red-600 text-sm mt-1">{error}</p></div></div>)}

        <div className="bg-white rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
          <div className="p-10 lg:p-16">
            <div className="mb-12 text-center"><h2 className="text-3xl font-black text-gray-800 uppercase tracking-tighter mb-4">Edit Struktur Organisasi</h2><p className="text-gray-600 text-lg">Perbarui informasi struktur organisasi masjid.</p></div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-10">
                    ...
                    <div className="space-y-4">
                      ...
                      <label htmlFor="foto-upload" className="block">
                        <div className="border-2 border-dashed border-gray-300 rounded-2xl p-10 text-center cursor-pointer hover:border-mu-green hover:bg-mu-green/5 transition-all duration-300 shadow-lg hover:shadow-xl">
                          {previewUrl ? (
                            <div className="space-y-6"><img src={previewUrl} alt="Preview" className="w-32 h-32 object-cover rounded-xl mx-auto shadow-lg" /><div><p className="text-lg font-semibold text-gray-800">{file?.name}</p><p className="text-sm text-gray-500">{(file?.size / 1024 / 1024).toFixed(2)} MB</p></div></div>
                          ) : formData.foto ? (
                            <div className="space-y-6"><img src={getFotoUrl(formData.foto)} alt="Foto Saat Ini" className="w-32 h-32 object-cover rounded-xl mx-auto shadow-lg" /><div><p className="text-lg font-semibold text-gray-700">Foto Saat Ini</p><p className="text-sm text-gray-500">Klik untuk mengganti foto</p></div></div>
                          ) : (
                            <div className="space-y-6"><Upload size={64} className="text-gray-400 mx-auto" /><div><p className="text-lg font-semibold text-gray-700">Klik untuk upload foto</p><p className="text-sm text-gray-500">PNG, JPG hingga 3MB</p></div></div>
                          )}
                        </div>
                      </label>
                    </div>
                  ...
                  <div className="space-y-6">
                    <h3 className="text-2xl font-bold text-gray-800 border-b-2 border-mu-green pb-3">Tanda Tangan (TTD)</h3>
                    <div className="space-y-4">
                      ...
                      <label htmlFor="ttd-upload" className="block">
                        <div className="border-2 border-dashed border-gray-300 rounded-2xl p-10 text-center cursor-pointer hover:border-mu-green hover:bg-mu-green/5 transition-all duration-300 shadow-lg hover:shadow-xl">
                          {previewTtdUrl ? (
                            <div className="space-y-6"><img src={previewTtdUrl} alt="Preview TTD" className="w-32 h-32 object-contain rounded-xl mx-auto shadow-lg bg-white" /><div><p className="text-lg font-semibold text-gray-800">{fileTtd?.name}</p><p className="text-sm text-gray-500">{(fileTtd?.size / 1024 / 1024).toFixed(2)} MB</p></div></div>
                          ) : formData.ttd ? (
                            <div className="space-y-6"><img src={getTtdUrl(formData.ttd)} alt="TTD Saat Ini" className="w-32 h-32 object-contain rounded-xl mx-auto shadow-lg bg-white" /><div><p className="text-lg font-semibold text-gray-700">TTD Saat Ini</p><p className="text-sm text-gray-500">Klik untuk mengganti TTD</p></div></div>
                          ) : (
                            <div className="space-y-6"><PenTool size={64} className="text-gray-400 mx-auto" /><div><p className="text-lg font-semibold text-gray-700">Klik untuk upload TTD</p><p className="text-sm text-gray-500">Background transparan (PNG) lebih disarankan</p></div></div>
                          )}
                        </div>
                      </label>
                    </div>
              ...
              <div className="flex flex-wrap justify-center gap-6 pt-12 mt-12 border-t-2 border-gray-200">
                <button type="button" onClick={() => navigate('/admin/struktur-organisasi')} className="flex items-center px-8 py-4 bg-gradient-to-r from-gray-200 to-gray-300 text-gray-700 rounded-2xl hover:from-gray-300 hover:to-gray-400 transition-all duration-300 font-semibold shadow-xl hover:shadow-2xl transform hover:-translate-y-2 hover:scale-105">Batal</button>
                <button type="submit" disabled={loading} className="flex items-center px-8 py-4 bg-mu-green text-white rounded-2xl hover:bg-green-700 transition-all duration-300 font-semibold shadow-xl hover:shadow-2xl transform hover:-translate-y-2 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"><Save size={22} className="mr-3" />{loading ? 'Menyimpan...' : 'Update Struktur'}</button>
              </div>
            </form>
          </div>
        </div>
