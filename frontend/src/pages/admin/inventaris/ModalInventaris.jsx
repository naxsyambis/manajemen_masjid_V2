...
const ModalInventaris = ({ show, onClose, onSubmit, form, setForm, isEdit }) => {
  ...
  const handleJumlahChange = (e) => {
    ...
    if (numberValue < 1) {
      setForm({ ...form, jumlah: '1' });
      return;
    }

    setForm({ ...form, jumlah: String(numberValue) });
  };

  const handleJumlahBlur = () => {
    const numberValue = Number(form.jumlah);
        ...
        <form onSubmit={onSubmit} className="p-8 space-y-6 overflow-y-auto max-h-[80vh] no-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2 space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest flex items-center gap-2">
                <Package size={12} className="text-mu-green" />
                Nama Barang
              </label>
              <input
                required
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest flex items-center gap-2">
                <Hash size={12} className="text-mu-green" />
                Jumlah
              </label>
              <input
                required
              ...
              <p className="text-[10px] font-bold text-gray-400 ml-2">
                Minimal kuantitas adalah 1 unit.
              </p>
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest flex items-center gap-2">
                <Info size={12} className="text-mu-green" />
                Kondisi
              </label>
              <select
                value={form.kondisi}
            ...
            <div className="md:col-span-2 space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest flex items-center gap-2">
                <MapPin size={12} className="text-mu-green" />
                Keterangan / Lokasi
              </label>
              <textarea
          ...
          <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-2xl flex items-start gap-3">
            ...
            <p className="text-[10px] text-yellow-800 font-bold leading-relaxed italic">
              Pencatatan inventaris yang akurat sangat penting untuk audit aset masjid.
              Pastikan jumlah dan lokasi barang diperbarui secara berkala.
            </p>
          </div>
